...
func NewChronicleTemplate(yFilename string, yFile *YamlFile) (ct *ChronicleTemplate, err error) {
	...
	// workaround until presets are working correctly:
	// Consider the "default" section an implicit preset
	// that is here applied to all content sections
	tmpDefault := NewContentEntry("default", yFile.Default)
	for id, entry := range ct.content {
		entry.AddMissingValuesFrom(&tmpDefault)
		ct.content[id] = entry
	}
